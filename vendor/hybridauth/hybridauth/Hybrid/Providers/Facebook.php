...
class Hybrid_Providers_Facebook extends Hybrid_Provider_Model {
    ...
    public $scope = "email, user_about_me, user_birthday, 
            user_hometown, user_website,user_friends, read_stream, offline_access, 
            publish_stream, read_friendlists,friends_about_me,friends_status,friends_birthday,
            friends_website,friends_hometown,friends_location,friends_work_history";
    ...
    function getUserContacts() {
        try {
            $response = $this->api->api('me/friends?fields=username,birthday,hometown,gender,location,picture,website,name,work');
        } catch (FacebookApiException $e) {
        ...
        foreach ($response["data"] as $item) {
            ...
            $uc->identifier = (array_key_exists("username", $item)) ? $item["username"] : "";                       
            $uc->displayName = (array_key_exists("name", $item)) ? $item["name"] : "";
            $uc->profileURL = "https://www.facebook.com/profile.php?id=" . $uc->identifier;
            $uc->photoURL = "https://graph.facebook.com/" . $uc->identifier . "/picture?width=150&height=150";
            $uc->location = $item['location']['name'];
            $uc->home = $item['hometown']['name'];
            $uc->company = $item['work'][0]['employer']['name'];

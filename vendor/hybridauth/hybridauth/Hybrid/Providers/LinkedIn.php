...
class Hybrid_Providers_LinkedIn extends Hybrid_Provider_Model {
    ...
    function getUserContacts() {
        try {
            // $response = $this->api->profile('~/connections:(id,first-name,last-name,picture-url,public-profile-url,date-of-birth,industry,location)');
            $response = $this->api->profile('~/connections:(id,first-name,last-name,headline,picture-url,email-address,location,industry,member-url-resources,date-of-birth)');
        } catch (LinkedInException $e) {
        ...
        foreach ($connections->person as $connection) {
            ...
            $uc->location = (string) $connection->{'location'}->name;
            $uc->home = '';
            $uc->company = '';
            $uc->birthday = '';
            $uc->jobTitle = (string) $connection->{'headline'};
    ...
    function sendMessageById($params) {

        $ids = $params['identity'];
        $subject = $params['subject'];
        $message = $params['message'];
        $response = $this->api->message($ids, $subject, $message, FALSE);
